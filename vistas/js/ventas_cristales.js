		...
		url:"ajax/cristales.ajax.php",
      	...
      	success:function(respuesta){
			  ...
			  '<div class="col-xs-3 ingresoPrecioCristal" style="padding-left:0px">'+
	              ...
	              '<input type="hidden" class="nuevoPrecioCristalBolivar" id="nuevoPrecioCristalBolivar" precioRealCristalBolivar="" name="nuevoPrecioCristalBolivar" readonly required>'+
			...
			$("#nuevaCantidadCristal").change(function(){
				var primerValor = $('#tasadeldia').val();
				var segundoValor = $('#nuevoPrecioCristal').val();
				var resultado = parseFloat(primerValor) * parseFloat(segundoValor);
				$('#nuevoPrecioCristalBolivar').val(resultado);
				$("#nuevoPrecioCristalBolivar").number(true, 2);
...
function listarCristales(){
	...
	for(var i = 0; i < descripcioncristal.length; i++){
		...
		listaCristales.push({ "id" : $(descripcioncristal[i]).attr("idCristal"), 
							  ...
							  "totalbolivar" : $(preciocristalbolivar[i]).val()
							})
